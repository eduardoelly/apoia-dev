...
export default function FormDonate({ creatorId, slug }: FormDonateProps) {
  ...
  async function handlePaymentResponse(checkout: { sessionId?: string, error?: string }) {
    ...
    if (!checkout.sessionId) {
      toast.error("Falha ao criar o pagamento, tente novamente")
      return
    }
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8 mt-2">
            ...
            <Button type="submit" disabled={form.formState.isSubmitting}>
              {form.formState.isSubmitting ? "Processando..." : "Fazer doação"}
            </Button>
          </form>
        </Form>
      </CardContent>
